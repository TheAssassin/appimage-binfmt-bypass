...
char* __abs_appimage_path() {
    ...
    if (libc_realpath == NULL) {
        libc_realpath = (char* (*) (const char*, char*)) dlsym(REAL_LIBC, "realpath");
    }
...
ssize_t readlink(const char* path, char* buf, size_t len) {
    ...
    // if necessary, we load the real read(...) symbol from libc
    static ssize_t (*libc_readlink)(const char*, void*, size_t) = NULL;
    if (libc_readlink == NULL) {
        libc_readlink = (ssize_t (*) (const char*, void*, size_t)) dlsym(REAL_LIBC, "readlink");
    }
    ...
    return libc_readlink(path, buf, len);
}
...
char* realpath(const char* name, char* resolved) {
    ...
    char* retval = libc_realpath(name, resolved);
...
int open(const char* file, int flags, ...) {
    ...
    if (libc_open == NULL) {
        libc_open = (int (*) (const char*, int)) dlsym(REAL_LIBC, "open");
    }
    ...
    int result = libc_open(file, flags);
